...
export default function useLocalTracks() {
  ...
  useEffect(() => {
    ...
    Video.createLocalTracks({
      ...
      .then((tracks) => {
        const fvideoTrack = tracks.find((track) => track.kind === "video")
        const faudioTrack = tracks.find((track) => track.kind === "audio")
        if (fvideoTrack) {
          setVideoTrack(fvideoTrack as LocalVideoTrack)
        }
        if (faudioTrack) {
          setAudioTrack(faudioTrack as LocalAudioTrack)
        }
      })
